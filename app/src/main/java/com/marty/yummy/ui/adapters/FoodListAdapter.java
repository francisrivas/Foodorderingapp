...
public class FoodListAdapter extends RecyclerView.Adapter<FoodListAdapter.RecyclerViewHolders> {
  ...
  public class RecyclerViewHolders extends RecyclerView.ViewHolder implements View.OnClickListener {
    ...
    public void onClick(View view) {
      if (view.getId() == R.id.food_card) {
        AppDatabase.getDatabase(view.getContext())
            .foodDetailsDao()
            .save(foodList.get(getAdapterPosition()));
  ...
  public RecyclerViewHolders onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    ...
    View layoutView =
        LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_food_item, null);
  ...
  public void onBindViewHolder(@NonNull final RecyclerViewHolders holder, int position) {
    ...
    LoadImage li =
        new LoadImage(
            holder.iFood, holder.iMinus, holder.iPlus, holder.tCount, holder.getAdapterPosition());
    handler.post(li);
  }
  ...
  private class LoadImage implements Runnable {
    ...
    public void run() {
      GlideApp.with(imageView.getContext())
          ...
          .into(imageView);
      int quantity =
          AppDatabase.getDatabase(imageView.getContext())
              .cartItemDao()
              .getCartCount(foodList.get(position).getName());
      foodList.get(position).setQuantity(quantity);
      tCount.setText(String.valueOf(quantity));
      iMinus.setOnClickListener(
